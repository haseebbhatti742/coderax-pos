...
function addItem() {
    ...
    if (product_id == "") {
        ...
        fetch("/admin/products/get", {
            ...
            if (data.status == "no") {
                document.getElementById("product_id_error").innerHTML = "Product Not Found";
...
function addItemInDatabase(product_id, product_name, unit_price) {
    fetch("/admin/home/addTempOrder", {
        method: "POST",
        body: JSON.stringify({ product_id, product_name, unit_price }),
        headers: new Headers({
            'Content-Type': 'application/json'
        }),
    }).then(data => data.json()).then(data => {
...
function deleteRow(temp_order_id) {
    fetch("/admin/home/deleteTempOrderRow", {
        method: "POST",
        body: JSON.stringify({ temp_order_id }),
        headers: new Headers({
            'Content-Type': 'application/json'
        }),
    }).then(data => data.json()).then(data => {
